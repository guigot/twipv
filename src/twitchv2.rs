use twitch_api2::{helix::channels::GetChannelInformationRequest, HelixClient, helix::streams::GetStreamsRequest, TwitchClient};
use twitch_oauth2::{AccessToken, AppAccessToken, ClientId, ClientSecret, client::reqwest_http_client, Scope, tokens::errors::TokenError, UserToken};
...
pub async fn appaccesstoken_twitch() -> AppAccessToken {
    ...
    let token =
    match AppAccessToken::get_app_access_token(twitch_oauth2::client::reqwest_http_client, client_id, client_secret, twitch_oauth2::Scope::all()).await {
        Ok(t) => t,
